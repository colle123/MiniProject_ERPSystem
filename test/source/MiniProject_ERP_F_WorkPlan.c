...
void Insert_Work_order_File() {
	...
	printf("납기일을 입력해주시오 : ");
	scanf("%d", &Due_Date); // 납기일 값 입력

	itoa(Due_Date, temp_int, 10);
	strcat(Parameter_Insert, temp_int);
	...
	if (_select(select_num_item, "item_NUMBER, item_NAME", &select_result_str) == -1) // 만든 양식을 토대로 품목목록에서 내부 칼럼정보를 선택해 받아옴
	{
		printf("%s\n", err_msg);
	...
	printf("\n");
	char select_customer[100] = "customer ="; // char* conditional =  "item_NAME = Select_item";
	strcat(select_customer, Customer_name); // item_NUMBER = CASE01

	
	if (_select(select_customer, "customer", &select_result_str) == -1) // 만든 양식을 토대로 품목목록에서 내부 칼럼정보를 선택해 받아옴
	{
		printf("%s\n", err_msg);
